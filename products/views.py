"""Страницы раздела изделий: список, карточка, состав, отчёты, импорт."""
...
def item_tree(request, pk):
    ...
    return render(request, "products/tree.html", {"item": item, "rows": rows})


def item_where_used(request, pk):
    item = get_object_or_404(Item, pk=pk)
    return render(request, "products/where_used.html", {
...
def item_summary(request, pk):
    ...
    return render(request, "products/summary.html", {"item": item, "rows": rows})
...
def item_edit(request, pk=None):
    ...
    return render(request, "products/form.html", {"form": form, "item": item})
...
def line_edit(request, pk, line_pk=None):
    ...
    return render(request, "products/line_form.html",
